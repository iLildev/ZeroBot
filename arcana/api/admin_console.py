...
from arcana.database.models import Bot, User, Wallet
...
async def get_user_identity(user_id: str, session: AsyncSession = Depends(get_session)):
...
async def admin_unverify_phone(user_id: str, session: AsyncSession = Depends(get_session)):
...
async def admin_unlink_session(user_id: str, session: AsyncSession = Depends(get_session)):
